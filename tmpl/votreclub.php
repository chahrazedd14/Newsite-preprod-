...
//echo $codeCRM;
	...
	echo '<div class="votrecluboffretop text-center">';
		...
		foreach($tags as $tag):
			...
			echo '<span style=" display: inline-flex;" class="p-2"><span style="border-radius:3px;" class="text-uppercase blanctext bluedbackground text-center p-2">'.$nom.'</span></span>';
...
<?php //print_r($item);
/*
20200701 - http://mmv.digitaltrafic.es/view.php?id=740
Galerie Votre Club Hiver sur Page établissement été
Enric: el problema es que se le pasa a la funcion printGalerie la url de invierno según el $item->alias del objeto $item.
Inspeccionando el elemento $item encuentro la url de verano en $item->urlsaison.
$item->saison => 1 tanto si se selecciona verano como se se selecciona invierno, de hecho el objeto item es el mismo independientemente :-(

la variable PHP $saison nos dice si es verano o invierno (2: verano / 1: invierno)
...
?>
-->
<script>/*<?php echo $item->urlsaison; ?>*/printGalerie(<?php echo $codeCRM; ?> , 'images/etablissements/<?php echo ($saison == 1) ? $item->alias : $item->urlsaison; ?>/onglet_principal','#galerieOngletPrincipal','', '<?php echo ($saison == 1) ? $item->alias : $item->urlsaison; ?>' , 'onglet_principal' );</script>
...
echo '<div class="" id="appartementsvotreclub">';
	...
	function htmlapartemanetsresidence($appartements, $stringSaison, $host, $lang, $hostimg){
		...
		foreach ($appartements as $appartement) :
			...
			echo '<div class="col-12 col-sm-12 item-residence';
			...
			echo '">';
				...
				echo '<div class="row mr-1">';
					...
					echo '<div class="col-12 col-lg-12">';
						...
						echo '</div>';
					echo '</div>';
					echo '<div class="col-12 col-lg-12  p-0 appartement_club">';
					echo '<button type="button" class="button-appartement btn" data-toggle="modal" data-target="#'.$appartement->Reference.'"><span class="button_info"> Plus de détails<span></button>';
						echo '<button type="button" class=" yellowbackground btn_appartement_tarifs" onclick="triggerClick(\'.tabTarifs a\');" >TARIFS ET DISPO </button>';
					echo '</div>';
				echo '</div>';
		...
		function htmlapartemanetshotel($appartements, $stringSaison, $host, $lang, $hostimg){
			...
			/*
			20200701 - http://mmv.digitaltrafic.es/view.php?id=743 - [page etab] problème d'affichage des chambres
			Enric:
			Hay que calcular el total de elementos que se van a mostrar,
			que en este caso no van a ser:
			$totalelemnt = count($appartements)/2;
			$totalelemnt = round($totalelemnt);
			Nota: Es aconsejable delegar el diseño de como se va a ve el layout o template al CSS en lugar de comprometer código PHP!!
			*/
			...
			endforeach;
			$totalelemnt = round($totalelement / 2);
			...
			foreach ($appartements as $appartement) :
				...
				echo '<div class="col-12 col-sm-12 item-hotel';
				//CALCULAMOS SI VA A LA COLUMNA DE LA DERECHA O A LA DE LA IZQUIERDA
				if ($con > $totalelemnt): 
					echo ' second-half ';
				endif; 
				echo '">';
					echo '<div class="row mr-1">';
						...
						echo '</div>';
						echo '<div class="col-12 col-lg-12  p-0 appartement_club">';
						echo '<button type="button" class="button-appartement btn" data-toggle="modal" data-target="#'.$appartement->Reference.'"><span class="button_info"> Plus de détails<span></span></span></button>';
						echo '<button type="button" class=" yellowbackground btn_appartement_tarifs" onclick="triggerClick(\'.tabTarifs a\');">TARIFS ET DISPO </button>';
						echo '</div>';
					echo '</div>';
